                              ...
                              uint8 *pRspDataLen, uint8 *pSendCS)
{
  ...
  switch(cmdID)
  {
    case MEASUREMENT_ERASE_HCI_EXT:
      {
	halEraseMsgData *pMsgData = (halEraseMsgData *)msg_ptr->utilNvWrite.pBuf;

	testEraseFlashHal(pMsgData->page, pMsgData->failFatal, pMsgData->failNonfatal);

	ICall_freeMsg(pMsgData);

	stat = SUCCESS;
      }
      break;
    case MEASUREMENT_STORAGE_HCI_EXT:
      {
        halWriteMsgDataWrite *pMsgData = (halWriteMsgDataWrite *)msg_ptr->utilNvWrite.pBuf;

        testWriteFlashHal(pMsgData->addr, pMsgData->len, pMsgData->data);
      ...
      break;
    case MEASUREMENT_STORAGE_HCI_EXT_READ:
      {
        halWriteMsgDataRead *pMsgData = (halWriteMsgDataRead *)msg_ptr->utilNvRead.pBuf;

        testReadFlashHal(pMsgData->page, pMsgData->offset, pMsgData->len, pMsgData->data);
