...
#define ST_TASK_STACK_SIZE                    1300
...
static uint8_t settingNewAdvertisingData = FALSE;
...
static uint8_t advertData[] =
{
  ...
  LO_UINT16(IRTEMPERATURE_CONTRACT_ID_UUID),
  HI_UINT16(IRTEMPERATURE_CONTRACT_ID_UUID),
  // track n trace / contract id
  0x00,
  0x00,
  0x00,
  0x00,
...
/*
static gapBondCBs_t SensorTag_bondCB =
{
  NULL,
  NULL
};
 ...
 * PUBLIC FUNCTIONS
 */
#define ADVERT_DATA_CONTRACT_ID_POS      7
#define ADVERT_DATA_SENSOR_COUNT_POS    27

static void SensorTag_setAdvertisingData(int queue)
{
  if (!queue || !Util_isActive(&periodicClock))
  {
  ...
  {
    // disable advertising first and set the advert data in the callback

    settingNewAdvertisingData = TRUE;
...
void SensorTag_updateAdvertDataSensorCount(uint8_t *sensorCount)
{
  memcpy(&advertData[ADVERT_DATA_SENSOR_COUNT_POS], sensorCount, 1);

  SensorTag_setAdvertisingData(TRUE);
}

void SensorTag_updateAdvertDataContractId(uint8_t *contractId, int len)
{
  memcpy(&advertData[ADVERT_DATA_CONTRACT_ID_POS], contractId, len);
...
static void SensorTag_init(void)
{
  ...
  // Setup the GAP Peripheral Role Profile
  {
    ...
    // start advertising again almost immediately after stopping
  ...
  GATTServApp_AddService(GATT_ALL_SERVICES);   // GATT attributes                      // Device Information Service
...
static void SensorTag_taskFxn(UArg a0, UArg a1)
{
  ...
  for (;;)
  {
    ...
    if (!!(events & ST_PERIODIC_EVT))
    {
      ...
      if (gapProfileState != GAPROLE_ADVERTISING && settingNewAdvertisingData)
      {
        settingNewAdvertisingData = FALSE;
        SensorTag_setAdvertisingData(FALSE);
      }
      ...
      {
        uint8_t isRecording;
        IRTemp_getParameter(SENSOR_CONF, &isRecording);
        if (isRecording == ST_CFG_SENSOR_ENABLE)
        {
...
static void SensorTag_processAppMsg(stEvt_t *pMsg)
{
  switch (pMsg->event)
  {
    ...
    case ST_READ_OUT_FLASH:
      SensorTagTmp_getMeasurements();
      break;

    case ST_DISABLE_RECORDING_ON_FLASH:
      SensorTagTmp_disableRecordingOnFlash();
      break;
    ...
    case ST_READ_OUT_FLASH_HASH:
      SensorTagTmp_getMeasurementHash();
      break;
 ...
 * @return  none
 */
void SensorTag_enqueueMsg(uint8_t event, uint8_t serviceID, uint8_t paramID)
{
  stEvt_t *pMsg;
