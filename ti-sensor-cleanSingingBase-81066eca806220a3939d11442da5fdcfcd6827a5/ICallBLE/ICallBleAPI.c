...
static bool matchHalCallTrue(ICall_ServiceEnum src, ICall_EntityID dest,
                               const void *msg)
{
  return true;
}

uint8 ICallBleTestRead(uint8_t page, uint16_t offset, uint16_t len, void *pBuf)
{
  ICall_UtilNvRead *msg =
    (ICall_UtilNvRead *)ICall_allocMsg(sizeof(ICall_UtilNvRead));

  halWriteMsgDataRead *msgContent = (halWriteMsgDataRead *)ICall_allocMsg(sizeof(halWriteMsgDataRead));

  if (msg && msgContent)
  {
    msgContent->page = page;
    msgContent->offset = offset;
    msgContent->len = len;
    msgContent->data = pBuf;

    msg->pBuf = msgContent;

    setICallCmdEvtHdr(&msg->hdr, HCI_EXT_UTIL_SUBGRP, MEASUREMENT_STORAGE_HCI_EXT_READ);

    // Send the message
    return sendWaitMatchCS(ICall_getEntityId(), msg, matchHalCallTrue);
  }
...
{
  uint8_t page;
  uint8_t *failNonfatal;
  uint8_t *failFatal;
} halEraseMsgData;

uint8 ICallBleEraseFlashPage(uint8_t page)
{
  ICall_UtilNvWrite *msg =
    (ICall_UtilNvWrite *)ICall_allocMsg(sizeof(ICall_UtilNvWrite));

  halEraseMsgData *msgContent = (halEraseMsgData *)ICall_allocMsg(sizeof(halEraseMsgData));

  if (msg && msgContent)
  {
    uint8_t failNonfatal = 0;
    uint8_t failFatal = 0;
    msgContent->page = page;
    msgContent->failNonfatal = &failNonfatal;
    msgContent->failFatal = &failFatal;

    msg->pBuf = msgContent;

    setICallCmdEvtHdr(&msg->hdr, HCI_EXT_UTIL_SUBGRP, MEASUREMENT_ERASE_HCI_EXT);

    // Send the message
    bStatus_t res = sendWaitMatchCS(ICall_getEntityId(), msg, matchHalCallTrue);
...
typedef struct
{
  ...
  uint16_t len;
  uint8_t *data;
} halWriteMsgDataWrite;

uint8 ICallBleTestWrite(uint32_t addr, uint16_t len, uint8_t *data)
{
  ICall_UtilNvWrite *msg =
    (ICall_UtilNvWrite *)ICall_allocMsg(sizeof(ICall_UtilNvWrite));

  halWriteMsgDataWrite *msgContent = (halWriteMsgDataWrite *)ICall_allocMsg(sizeof(halWriteMsgDataWrite));

  if (msg && msgContent)
  {
    msgContent->addr = addr;
    msgContent->len = len;
    msgContent->data = data;

    msg->pBuf = msgContent;

    setICallCmdEvtHdr(&msg->hdr, HCI_EXT_UTIL_SUBGRP, MEASUREMENT_STORAGE_HCI_EXT);
